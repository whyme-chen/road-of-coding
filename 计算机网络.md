...
4. 根据应用需求的不同，因特网的传输层为应用层提供了两种不同的运输协议，即面向连接的TCP（Transmission Control Protocol）和无连接的UDP（User Datagram Protocol）。
...
1. DNS（Domain Name System）：主要是方便人们记忆
