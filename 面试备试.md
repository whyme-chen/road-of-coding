...
1. 理解：每次获取数据的时候不会上锁，但更新数据那一刻会判断数据是否被更新过了，如果数据的值跟自己预期一样的话，那么就可以正常更新数据。
2. 场景：这种思想应用到实际场景的话，可以用版本号机制和CAS算法实现。
3. 

### 悲观锁

1. 理解：每次操作共享资源时都会上锁，直到整个数据处理过程结束。
2. 常见使用场景：数据库很多地方就用到了这种锁机制，比如行锁，表锁，读锁，写锁等。
3. 实现：悲观锁的实现往往依靠数据库本身的锁功能实现。Java程序中的synchronized和ReentrantLock等实现的锁也均为悲观锁。
