...
3. 同步与异步
   ...
   * 异步：不需要等待结果返回，就能继续运行
4. 并发问题
   * 在Java中，常说的并发问题指的是多线程环境下可能出现的一系列问题，包括但不限于**竞态条件**（Race Condition）、**死锁**（Deadlock）、**活锁**（Livelock）、**资源争夺**（Resource Contention）等。
...
* 阻塞式的解决方案：
  * 锁机制：**synchronized**, **Lock**，**ReentrantReadWriteLock**
  * 信号量：**Semaphore**
* 非阻塞式的解决方案：
...
1. 核心：要对**共享**和**可变**状态的访问操作进行管理
2. 解决方案：采用同步机制协同线程对变量的访问（java中主要同步机制：synchronized关键字，volatile类型变量，显式锁以及原子变量）
